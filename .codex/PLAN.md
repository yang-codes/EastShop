...
## Project Docs
- `.codex/TODO.md`：当前任务和测试清单。
- `.codex/ARCHITECTURE.md`：技术结构和数据表设计。
- `.codex/API.md`：前端服务接口、Edge Function 目标接口和数据类型。
- `.codex/CHANGELOG.md`：已完成变更记录。
- `.codex/sql-docs/SUPABASE_SQL_SETUP.md`：Supabase 建表、RLS、表权限、管理员授权和常见错误处理 SQL。

## Product Scope
- Telegram、Instagram、Web 共用同一个商城前台。
- URL 支持 `source=telegram|instagram|web` 来源识别。
...
- 下单页填写客户信息、详细地址、社交账号、备注，并支持浏览器定位和 Geoapify 地址反查。
...
- DeepL API Free 用于后台商品翻译辅助，翻译结果必须允许人工修改。
...
- Supabase 创建表、索引、Storage bucket、RLS 策略、表访问权限和 `submit-order` Edge Function。
- 公开仓库不能提交 `.env.local`、Supabase service role key、Telegram Bot Token、DeepL key 或数据库连接密码。
...
- 后台必须支持手机和电脑浏览器。
- Instagram 只作为普通网页入口兼容，不提供可信平台用户验证。
